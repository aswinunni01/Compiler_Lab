...
{	/* Class Assignment */
	if (l->Ctype != NULL || r->Ctype != NULL && (r->nodetype != MethodCall && r->nodetype != SelfFnNode))
	{ /* Class assign if a=b. not if a=ID.b() / a = ID.b.b() / a = self.b() / a = self.b.b()*/
...
{ /* Code Gen for AST */
	...
	case MethodBlock:		/* Connector for methods */
	{
		codeGen(t->left, fptr);
		freeAllReg();
		codeGen(t->right, fptr);
	...
	case FnBlock:		/* Connector for methods */
	{
		codeGen(t->left, fptr);
		freeAllReg();
		codeGen(t->right, fptr);
	...
	case MethodNode:		/* Node for method body */
	{
		fprintf(fptr, "F%d:\n", t->val);
	...
	case FnNode:		/* Node for Function body */
	{
		fprintf(fptr, "F%d:\n", t->Gentry->flabel);
		fprintf(fptr, "PUSH BP\n");
		fprintf(fptr, "MOV BP, SP\n");
	...
	case MethodCall:
	case SelfFnNode:
	{ //self.method_name()  - method call node
		int n_reg = -1;
		for (int i = 0; i < count; i++)
		{
			fprintf(fptr, "PUSH R%d\n", i);
...
{ /* Stores location of a variable to a register and returns the register */
	if (t->Lentry != NULL)
	{ /* Case if variable is local variable / Parameter */
		switch (t->nodetype)
		{
	...
	{ /* Case if variable is global variable */
...
void checkoverriding(struct Memberfunclist* parent_fn , char *name, struct Typetable *type, struct Paramstruct *Paramlist){
	if((strcmp(parent_fn->name, name)==0) && (parent_fn->type == type)){
			struct Paramstruct *parent_param = parent_fn->paramlist;
			struct Paramstruct *temp  = Paramlist;
			while(parent_param != NULL && temp != NULL){
				if((parent_param->name != temp->name) || (parent_param->type != temp->type)){
						yyerror("Function overriding requires signature of both functions to match exactly\n");
						exit(1);
				}
				else{
...
void Class_Minstall(struct Classtable *cptr, char *name, struct Typetable *type, struct Paramstruct *Paramlist)
{
	...
	if (CMLookup(cptr, name) != NULL)
	{
		...
		checkoverriding(CMLookup(cptr->Parentptr, name), name, type, Paramlist);
