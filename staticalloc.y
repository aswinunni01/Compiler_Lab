...
%type <no> E PLUS MINUS MUL DIV Stmt Stlist InputStmt OutputStmt AsgStmt ID NUM Ifstmt NE EQU LT LTE GT GTE Whilestmt GidList FdefBlock Fdef Body MainBlock ArgList Retstmt IdList LDecl Field LIT INIT ALLOC FREE MethodDefns Cname ClassDef ClassDefList Method
...
ClassDefBlock : CLASS ClassDefList ENDCLASS	{ printclasstable(); }
	      | CLASS ENDCLASS			{  }
;
...
MethodDefns	: MethodDefns Fdef		{  $$ = createTree(NULL,FnBlock, NULL,FnBlock, NULL, $1,NULL, $2); }
	    	| Fdef				{  $$ = $1; }
		;
...
LdeclBlock : DECL LDecList ENDDECL	{	if (Cptr != NULL){
	   						Linstall("self", TLookup("VOID"), ++lloc);
						}
					}
	   | DECL ENDDECL		{
						if (Cptr != NULL){
...
Stmt	: InputStmt ';'	{ $$ = $1; }
	...
	| E EQ NEW '(' Cname ')' ';' { //$$ = createNewNode($1) 
				}
;
